...
export default function useCreditCardItems(owid: string, creditCardId: string, sheetId: string) {
  ...
  const deleteCreditCardItem = async (itemId: string) => {
    try {
      const resp = await axios.delete(`/credit_card_items/${itemId}?sheetid=${sheetId}`);
  ...
  const createCreditCardItem = async (c: INewCreditCardItem) => {
    try {
      const resp = await axios.post('/credit_card_items', {
        sheetId: sheetId,
        ownerId: c.ownerId,
        name: c.name,
  ...
  const updateCreditCardItem = async (c: ICreditCardItem) => {
    try {
      ...
      const resp = await axios.put(
        `/credit_card_items/${c.id}?owid=${c.ownerId}&sheetid=${sheetId}&creditCardId=${c.creditCardId}`,
        {
          name: c.name,
          description: c.description,
          amount: c.amount,
